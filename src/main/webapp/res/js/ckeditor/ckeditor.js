...
(function() {
        ...
        a.htmlParser.fragment.fromHtml = function(o, p, q) {
            ...
            function z(A, B, C) {
                ...
                if (p && (!B.type || B.name == 'body')) {
                    var E, F;
                    if (A.attributes && (F = A.attributes['data-cke-real-element-type']))
                        E = F;
                    else
                        E = A.name;
                ...
                if (A._.isBlockLike && A.name != 'pre') {
                    ...
                    if (H && H.type == 3)
                        if (!(I = e.rtrim(H.value)))
                            A.children.length = G - 1;
                        else
                            H.value = I;
                }
                B.add(A);
                ...
                while (1) {
                    ...
                    if (!E.isUnknown && !v.isUnknown && !G[A]) {
                        if (v.isOptionalClose)
                            ...
                            if (!(I && I.name == 'li'))
                                z(I = new a.htmlParser.element('li'), v);
                            !E.returnPoint && (E.returnPoint = v);
                            v = I;
                        } else if (A in f.$listItem && F != A)
                            r.onTagOpen(A == 'li' ? 'ul' : 'dl', {}, 0, 1);
                        else if (F in l && F != A) {
                            !E.returnPoint && (E.returnPoint = v);
                            v = v.parent;
                        } else {
            ...
            r.onText = function(A) {
                ...
                y();
                x();
                if (p && (!v.type || v.name == 'body') && e.trim(A))
                    this.onTagOpen(p, {}, 0, 1);
                if (!w)
                    A = A.replace(/[\t\r\n ]{2,}|[\t\r\n]/g, ' ');
        ...
        function I(T) {
            return T.replace(B, function(U, V, W) {
                return '<' + V + W.replace(C, function(X, Y) {
                    if (W.indexOf('data-cke-saved-' + Y) == -1)
    ...
    i.keystrokes = [[4000 + 121, 'toolbarFocus'], [4000 + 122, 'elementsPathFocus'], [2000 + 121, 'contextMenu'], [1000 + 2000 + 121, 'contextMenu'], [1000 + 90, 'undo'], [1000 + 89, 'redo'], [1000 + 2000 + 90, 'redo'], [1000 + 76, 'link'], [1000 + 66, 'bold'], [1000 + 73, 'italic'], [1000 + 85, 'underline'], [4000 + (c || b.webkit ? 189 : 109), 'toolbarCollapse'], [4000 + 48, 'a11yHelp']];
    ...
    (function() {
        ...
        j.list = {listToArray: function(E, F, G, H, I) {
                ...
                for (var J = 0, K = E.getChildCount(); J < K; J++) {
                    var L = E.getChild(J);
                    if (L.type == 1 && L.getName() in f.$list)
                        j.list.listToArray(L, F, G, H + 1);
                ...
                while (1) {
                    ...
                    if (R.indent == N) {
                        ...
                        for (var S = 0; S < R.contents.length; S++)
                            O.append(R.contents[S].clone(1, 1));
                        M++;
                        ...
                        if (m[R.grandparent.getName()]) {
                            ...
                            O = new d.documentFragment(J);
                        for (S = 0; S < R.contents.length; S++)
                            O.append(R.contents[S].clone(1, 1));
                        if (O.type == 11 && M != E.length - 1) {
                            var W = O.getLast();
                            if (W && W.type == 1 && W.getAttribute('type') == '_moz')
                                W.remove();
                            if (!(W = O.getLast(q) && W.type == 1 && W.getName() in f.$block))
    ...
    (function() {
        ...
        function q(C) {
            ...
            if (c) {
                ...
                I.document.$.execCommand('inserthtml', false, C);
            if (b.webkit) {
                D = I.getSelection();
                D.scrollIntoView();
            }
    ...
    (function() {
        ...
        a.dialog = function(Q, R) {
            ...
            if (S.onOk)
                this.on('ok', function(ap) {
                    ...
                    if (S.onOk.call(this, ap) === false)
                        ap.data.hide = false;
                });
            if (S.onCancel)
                this.on('cancel', function(ap) {
                    if (S.onCancel.call(this, ap) === false)
                        ap.data.hide = false;
                });
            ...
            function af() {
                ...
                ap.sort(function(as, at) {
                    if (as.tabIndex != at.tabIndex)
                        return at.tabIndex - as.tabIndex;
                    else
                        return as.focusIndex - at.focusIndex;
                });
                var aq = ap.length;
            ...
            this.on('show', function() {
                ...
                if (b.opera || b.gecko && b.mac)
                    ak.on('keypress', aj, this);
            });
            this.on('hide', function() {
                ak.removeListener('keydown', ai);
                if (b.opera || b.gecko && b.mac)
                    ak.removeListener('keypress', aj);
            });
        ...
        a.dialog.prototype = {destroy: function() {
                ...
                if (a.dialog._.currentTop === null) {
                    ...
                    for (var T in {keyup: 1,keydown: 1,keypress: 1})
                        Q.on(T, P);
                } else {
        ...
        var H = {}, I = function(Q) {
            ...
            if (Q.data.getKeystroke() in O)
                Q.data.stopPropagation();
        };
        (function() {
            k.dialog = {uiElement: function(Q, R, S, T, U, V, W) {
                    ...
                    if (this.accessKeyUp && this.accessKeyDown && R.accessKey)
                        K(this, Q, 'CTRL+' + R.accessKey);
                    var aj = this;
                    Q.on('load', function() {
                        if (aj.getInputElement())
                            aj.getInputElement().on('focus', function() {
        ...
        (function() {
            ...
            a.dialog.validate = {functions: function() {
                    return function() {
                        ...
                        if (X < arguments.length && typeof arguments[X] == 'number')
                            V = arguments[X];
                    ...
                    if (!ak || aa[ak] && !am && (!al || Z) && (ae.getPosition(ad) | 4 | 0 | 8) == 4 + 0 + 8 && (!X.childRule || X.childRule(ae))) {
                        ...
                        if (an && ((an.getDtd() || f.span)[W] || Y) && (!X.parentRule || X.parentRule(an))) {
                            ...
                            if (aj == 3 || al || aj == 1 && !ae.getChildCount()) {
                                var ao = ae, ap;
                                while ((ai = !ao.getNext(q)) && (ap = ao.getParent(), aa[ap.getName()]) && (ap.getPosition(ac) | 2 | 0 | 8) == 2 + 0 + 8 && (!X.childRule || X.childRule(ap)))
                                    ao = ap;
                                af.setEndAfter(ao);
                            }
                        } else
