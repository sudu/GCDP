...
function EditAreaLoader(){
	...
	t.default_settings= {
		...
		,min_width: 400
		,min_height: 125
		,replace_tab_by_spaces: 4 //tab缩进4个空字符的距离
		...
		,ifeng_script_debug: false//是否打开ifeng脚本调试功能
	...
	t.advanced_buttons = [
            ...
            ['release','release.jpg','release',false],
			...
			['format_code', 'format_code.png', 'format_code', false],
			['ifeng_lib', 'logo_chanel_ifeng.gif', 'load_ifeng_lib', false]
		];
	...
	start : function(id){
		...
		if(!t.iframe_script){
			...
			for(i=0; i<t.sub_scripts_to_load.length; i++)
				t.iframe_script+='<script language="javascript" type="text/javascript" src="'+ t.baseURL + t.sub_scripts_to_load[i] +'.js?v=20131012"></script>';
		}
	...
	*/	
	...
	toggle_off : function(id){
		...
		{	
			...
			//t.style.display="inline"; // cds modify 保证退出全屏时能恢复到初始尺寸
	...
	set_editarea_size_from_textarea : function(id, frame){	
		...
		width	= Math.max(editAreas[id]["settings"]["min_width"], elem.offsetWidth,parseInt(elem.style.width))+"px";
		height	= Math.max(editAreas[id]["settings"]["min_height"], elem.offsetHeight,parseInt(elem.style.height))+"px";
	...
	show : function(id){
		...
		{
			//elem.style.display= "inline";// cds modify 保证退出全屏时能恢复到初始尺寸
