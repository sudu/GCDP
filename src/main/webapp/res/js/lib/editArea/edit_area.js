...
******/
	...
	function EditArea(){
		...
		t.tab_nb_char= 4;	//nb of white spaces corresponding to a tabulation
		if(t.isOpera)
			t.tab_nb_char= 3;
	...
	EditArea.prototype.manage_size= function(onlyOneTime){
		...
		if(editAreas[this.id]["displayed"]==true && this.textareaFocused)
		{
			...
			if(this.last_selection["nb_line"] >= this.line_number)
			{
				...
				//是否打开ifeng脚本调试
				var ifeng_debug = parent.editAreas[this.id]['settings']["ifeng_script_debug"]; 
				
				for( i = start+1; i < end; i++ )
				{
					if(ifeng_debug){
						newLines+='<div id="line_'+ i +'" style="cursor:pointer" onclick="parent.Editor.debug.lineno_div_click('+i+')">'+i+"</div>";
					}else{
	...
	EditArea.prototype.execCommand= function(cmd, param){
		...
		switch(cmd){
			...
			case "file_close":
				...
				break;
            case "release":
                if(this.settings["EA_release_callback"].length>0)
                    return eval("parent."+this.settings["EA_release_callback"]+"(param);");
                break;
