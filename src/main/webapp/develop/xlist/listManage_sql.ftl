...
var listMgr2 = {
								...
								handler:function(obj){
									try{
										...
										if(cfg.sourceType != 'sql'){
											Ext.Msg.show({
												   title:'警告提示',
												   msg: '不能把数据源类型【' + cfg.sourceType +'】的配置导入自定义SQL数据源类型的页面，请选择相应的数据源类型，再进行导入！',
												   buttons: Ext.Msg.OK,
												   animEl: 'elId',
												   minWidth:420,
				...
				items:[{
						...
						items:[{
							...
							items:[{
								...
								name:'txtSql',
								anchor:'60%',
								emptyText:"自定义sql不允许存在order by 和 limit",
								fieldNote:"自定义sql不允许存在order by 和 limit",
								value:LPCFG.sql?LPCFG.sql:''
							},
							{
								xtype:'xTextField',
								fieldLabel:'默认排序',
								name:'defaultSort',
								anchor:'60%',
								emptyText:"eg.{'field':'id','order':'desc'}",
								fieldNote:'eg.{"field":"id","order":"desc"}按ID倒序排序',
							...
							},
							{
								...
								name:'neededFields',
								anchor:'60%',
								emptyText:"用,分割.eg.id,title,area",
								fieldNote:"用,分割.eg. id,title,area",
							...
							{//fieldLabel:'模板',
								...
								items:[{
									...
									items:[{
										...
										editable : false,
										readOnly:true,
										value: LPCFG.template?LPCFG.template:'listTemplate_sql_1',//保持兼容性
										width:200,
		...
		hisListMgr.initHistory({
			...
			reSetFormPanels : function(cfg){
				if(cfg.sourceType != 'sql'){
					Ext.Msg.show({
						   title:'警告提示',
						   msg: '不能把数据源类型【' + cfg.sourceType +'】的配置回滚到自定义SQL数据源类型的页面，请选择相应的数据源类型，再进行历史回滚！',
						   buttons: Ext.Msg.OK,
						   animEl: 'elId',
						   minWidth:420,
						   icon: Ext.MessageBox.ERROR 
	...
	initDbSearchSetting:function(){
		...
		store= new Ext.data.Store({
			...
			reader : new Ext.data.JsonReader({
				autoLoad:true,
				fields: ["field","title","enable","dataSourceType","ctrl","width","dataSource","op"]
			})
		});
	...
	collectListCfg:function(frm){
		var cfg={
			...
			sql:frm.txtSql,
			defaultSort:frm.defaultSort?Ext.decode(frm.defaultSort):null,
			...
			columns:[],
			enableSearch:frm.chkSearch?true:false,
			search:[]
		};
		...
		//////cfg.search///////
		var search=[];
		items = listMgr2.gridSearch.store.data.items;
		for(var i=0;i<items.length;i++){
			var item = items[i].data;
			search.push(item);
		}
